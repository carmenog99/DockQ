...
def find_atoms(sample_model, ref_model, interface, ligand_interface, receptor_interface, atom_for_sup):
	ref_atoms = []
	ref_ligand_atoms = []
	ref_receptor_atoms=[]
	sample_atoms = []
	sample_ligand_atoms=[]
	sample_receptor_atoms=[]
	

	common_interface=[]
	
	common_ligand_interface=[]
	
	common_receptor_interface=[]
...
#	print atoms_def_in_both
	...
	for ref_chain in ref_model:
		...
		for ref_res in ref_chain:
			...
			if key in common_receptor_interface:
			  # Check if residue number ( .get_id() ) is in the list
			  # Append CA atom to list
				#print key  
				for a in atom_for_sup:
...
#   inter_out = subprocess.getoutput(cmd_interface)
	...
	#I just added a new variable to save the ligand interface residues only, in order to carry out the IL-RMSD. 
	...
	for i in interface:
		lig_ident=i[-1:]
		if lig_ident=="L":
			ligand_interface.append(i)
	
	
	for i in interface:
		rec_ident=i[-1:]
		if rec_ident=="R" or rec_ident=="A":
	...
	#sup=superpose(ref_receptor_atoms, sample_receptor_atoms)
	#il_rmsd=calcRMSD(chain_ref["L"], chain_sample["L"])
	...
	sup=SVDSuperimposer()
	#sup.set(ref_receptor_atoms, sample_receptor_atoms)
	Lrms = sup._rms(coord1,coord2) #using the private _rms function which does not superimpose
	#Lrms=calcRMSD(coord1, coord2)
	
	#print(coord2)
	#print(ref_receptor_atoms)
	#print(sample_receptor_atoms)
	
	#print(ref_receptor_atoms)
	#print(sample_receptor_atoms)
	
	"""
	coord_ref_int=np.array([atom.coord for atom in ref_receptor_atoms])
	#print(coord_ref_int)
	coord_mod_int=np.array([atom.coord for atom in sample_receptor_atoms])
	#print(coord_mod_int)
	"""
	
	#IL-RMSD. We are going to superimpose the interface atoms from the reference receptor and then we are going to measure the RMSD of both ligands, the model and the receptor. This is the interface ligand RMSD. 
	super_imposer_il = Bio.PDB.Superimposer()
...
def get_pdb_chains(pdb): #I would say that this function is used to obtain the id of each pdb file. 
	pdb_parser = Bio.PDB.PDBParser(QUIET = True) #This function parser one pdb file and return a structure. 
	...
	for c in pdb_struct:
		#print(c)
		chain.append(c.id)
	return chain # a list with the ids of the chains.
#ATOM   3312  CA
...
#		print chain
	...
	f.close();
	#print outfile
	exec_path=os.path.dirname(os.path.abspath(sys.argv[0]))   #gives the path of DockQ.py file.  
	cmd=exec_path + '/scripts/renumber_pdb.pl ' + outfile
	os.system(cmd) #Executes cmd variable, which is a command to execute the temporal file with the perl script. 
	os.remove(outfile) #Deletes the temporal file. 
	return outfile +'.renum'
	
def main():

	#This is used to obtain the arguments given in the command line. Here we have to read at least the name of the model file and the native file. 
...
#	parser.add_argument('-comb',default=False,action='store_true',help='use all cyclicchain permutations to find maximum DockQ (number of comparisons is n!*m! = 24*24 = 576 for two tetramers interacting')
	...
	#In this part, they check if the bio version is lower than the one needed. If it is, we need to upload the version, so the code stop running. 
